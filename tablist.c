...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
      printf("Usage:  [TABLIST_WIDTH=80] tablist filename[ext][col filter][row filter] \n");
      printf("\n");
    ...
    if ((val=getenv("TABLIST_WIDTH"))) {
      if (1==sscanf(val,"%d",&colwidth)) {
        fprintf(stderr,"### Setting column width to %d\n", colwidth);
      } else {
        fprintf(stderr,"### Bad value for row width (%s) in TABLIST_WIDTH environment variable\n", val);
        return 1;
      }
    }
    ...
    if (!fits_open_file(&fptr, argv[1], READONLY, &status))
    {
      ...
      else  
      {
        ...
        /* find the number of columns that will fit within [colwidth] characters */
        while(lastcol < ncols) {
          linewidth = 0;
          firstcol = lastcol+1;
